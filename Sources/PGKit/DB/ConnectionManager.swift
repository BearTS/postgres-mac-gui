...
public actor ConnectionManager {
    ...
    public static let applicationName = "Postgres Manager"
    ...
    public init(logger: Logger = Logger(label: "PGKit.ConnectionManager")) {
        self.logger = logger
    }
    ...
    public func client(database: String) throws -> PostgresClient {
        ...
        // The pool services no requests until run() is executing.
    ...
    public func query(_ query: PostgresQuery, database: String? = nil) async throws -> [PostgresRow] {
        let client = try client(database: database ?? Self.adminDatabase)
    ...
    public func executeInTransaction(_ statements: [String], database: String) async throws {
        let client = try client(database: database)
